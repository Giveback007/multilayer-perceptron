...
const errorFunctions: { [key: string]: ErrorFunction } = {
    absolute: (output: output, target: target) => output - target,
    square: (output: output, target: target) => (output - target) ** 2 / 2,
};

// * teacher * - scores the output of the NN to the target
export function findOutputErrors(outputs: output[], targets: target[]): error[] {
    return targets.map((t, i) => errorFunctions.square(outputs[i], t));
}
